...
class InventoryMenu extends ItemMenu
{
	...
	function InitExtensions()
	{
		...
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu InitExtensions()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onExitMenuRectClick()");
		StartMenuFade();
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu StartMenuFade()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onFadeCompletion()");
		if (_bMenuClosing) {
			GameDelegate.call("CloseMenu", []);
		}
	}
	...
	function onShowItemsList(event)
	{
		...
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onShowItemsList()");
		if (event.index != -1) {
	...
	function onItemHighlightChange(event)
	{
		...
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onItemHighlightChange()");
		if (event.index != -1) {
			UpdateBottomBarButtons();
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu UpdateBottomBarButtons()");
	...
	function onHideItemsList(event)
	{
		...
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onHideItemsList()");
		BottomBar_mc.UpdatePerItemInfo({type:InventoryDefines.ICT_NONE});
	}

	function onItemSelect(event)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onItemSelect()");
	...
	function DropItem()
	{
                if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu DropItem()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onQuantityMenuSelect()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onMouseRotationFastClick()");
		GameDelegate.call("CheckForMouseEquip", [aiMouseButton], this, "AttemptEquip");
	}

	function onItemCardListPress(event)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onItemCardListPress()");
		GameDelegate.call("ItemCardListCallback", [event.index]);
	}

	function onItemCardSubMenuAction(event)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu onItemCardSubMenuAction()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu SetPlatform()");
		InventoryLists_mc.ZoomButtonHolderInstance.gotoAndStop(1);
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("InventoryMenu ItemRotating()");
