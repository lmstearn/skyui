...
class ItemMenu extends MovieClip
{
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("<========================ITEMMENU INITEXTENSIONS==================================" + "\n");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onConfigLoad()");
		_config = event.config;
	}

	function PositionElements()
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu PositionElements()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu SetPlatform()");
	...
	{
	if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onMouseWheel()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onExitMenuRectClick()");
		GameDelegate.call("CloseMenu",[]);
	}

	function onCategoryChange(event)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onCategoryChange()");
	}
	...
	*/
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onShowItemsList()");
		onItemHighlightChange(event);
	}

	function onHideItemsList(event)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onHideItemsList()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onItemSelect()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onQuantityMenuSelect()");
		GameDelegate.call("ItemSelect",[event.amount]);
	}

	function UpdatePlayerInfo(aUpdateObj)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu UpdatePlayerInfo()");
		BottomBar_mc.UpdatePlayerInfo(aUpdateObj,ItemCard_mc.itemInfo);
	}

	function UpdateItemCardInfo(aUpdateObj)
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu UpdateItemCardInfo()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onItemCardSubMenuAction()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu ConfirmSelectedEntry()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onMouseRotationStart()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onMouseRotationStop()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu onMouseRotationFastClick()");
	...
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu ToggleMenuFade()");
		if (bFadedIn) {
	...
	function SetFadedIn()
	{
		if (DEBUG_LEVEL > 0) _global.skse.Log("ItemMenu SetFadedIn()");
		bFadedIn = true;
		InventoryLists_mc.ItemsList.disableSelection = false;
		InventoryLists_mc.ItemsList.disableInput = false;
		InventoryLists_mc.CategoriesList.disableSelection = false;
